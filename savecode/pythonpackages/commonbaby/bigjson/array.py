...
class Array:
    ...
    def _read_all(self, to_python=False):
        ...
        while True:
            ...
            if to_python:
                python_list.append(
                    self.reader.read(read_all=True, to_python=True))
            else:
    ...
    def __getitem__(self, index):
        ...
        # Try from lookup table
            ...
            lookup_table_index = (index - self.index_lookup_multiplier
                                  ) / self.index_lookup_multiplier
            ...
            seek_index = (
                lookup_table_index + 1) * self.index_lookup_multiplier
        ...
        while True:
            ...
            if seek_index == (
                    len(self.index_lookup) + 1) * self.index_lookup_multiplier:
                ...
                if len(self.index_lookup) > Array._MAX_INDEX_LOOKUP_LENGTH:
                    self.index_lookup = [
                        pos for i, pos in enumerate(self.index_lookup)
                        if i % 2 == 1
                    ]
