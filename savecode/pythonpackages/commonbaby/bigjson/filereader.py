...
class FileReader:
            ...
            while True:
                ...
                if c == u'\\':
                    ...
                    if c == u'"':
                        ...
                        string += ('\\u' + unicode_bytes
                                   ).encode('ascii').decode('unicode_escape')
                    else:
                        raise Exception(
                            u'Unexpected {} in backslash encoding!'.format(c))
    ...
    def _ensure_readbuf_left(self, minimum_left):
        if len(self.readbuf) - self.readbuf_read >= minimum_left:
            return
        read_amount = max(minimum_left, FileReader._READBUF_CHUNK_SIZE) - (
            len(self.readbuf) - self.readbuf_read)
        self.readbuf_pos += self.readbuf_read
        self.readbuf = self.readbuf[self.readbuf_read:] + self.file.read(
            read_amount).decode('utf-8')  # ('ascii')
        self.readbuf_read = 0
    ...
    def _seek(self, pos):
        ...
        if pos >= self.readbuf_pos and pos <= self.readbuf_pos + len(
                self.readbuf):
