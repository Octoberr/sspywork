...
from ..zgrab2parser import Zgrab2ParserNtp


class Zgrab2ScannerNtp(Zgrab2ScannerBase):
    """zgrab2 ntp scanner"""

    def __init__(self, zgrab_path: str):
        Zgrab2ScannerBase.__init__(self, "zgrab2ntp")
        self._parser: Zgrab2ParserNtp = Zgrab2ParserNtp()
    ...
    def get_banner_ntp(
        ...
        *args,
        ...
        """scan ntp services and get the banner"""
        try:
            ...
            self._parser.parse_banner_ntp(task, level, pinfo_dict, outfi)

        except Exception:
            self._logger.error("Scan ntp error: {}".format(traceback.format_exc()))
    ...
    def _scan_ntp(
        ...
        *args,
        ...
        try:
            ...
            enhanced_args.append("ntp")
            enhanced_args.append(f"-p {port}")
            # zgrab2 ntp -p 123 -n ntp -t 10 -o ./mt1.json -f ./mtip.txt
            enhanced_args.append("-n ntp")
            ...
            # outfi = os.path.join(self._tmpdir, "{}_{}.ntp".format(task.taskid, port))
            with self._outfile_locker:
                outfi = os.path.join(
                    self._tmpdir, "{}_{}.ntp".format(str(uuid.uuid1()), port)
                )
            ...
            try:
                ...
                if exitcode != 0:
                    raise Exception(f"Scan Ntp error: {stdout}\n{stderr}")
                self._logger.info(
                    f"Scan Ntp exitcode={str(exitcode)}\ntaskid:{task.taskid}\nbatchid:{task.batchid}\nport:{port}"
                )
            finally:
