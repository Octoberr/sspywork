...
from ..zgrab2parser import Zgrab2ParserMongodb


class Zgrab2ScannerMongodb(Zgrab2ScannerBase):
    """zgrab2 http scanner"""

    def __init__(self, zgrab_path: str):
        Zgrab2ScannerBase.__init__(self, "zgrab2mongodb")
        self._parser: Zgrab2ParserMongodb = Zgrab2ParserMongodb()

    def get_banner_mongodb(
        self,
        task: IscanTask,
        level,
        pinfo_dict,
        ...
        *args,
        ...
        try:
            ...
            self._parser.parse_banner_mongodb(task, level, pinfo_dict, outfi)

        except Exception:
            self._logger.error("Scan mongodb error: {}".format(traceback.format_exc()))
    ...
    def _scan_mongodb(
        ...
        *args,
        ...
        try:
            ...
            enhanced_args.append("mongodb")
            enhanced_args.append(f"-p {port}")
            # zgrab2 mongodb -p 27017 -n mongodb -t 10 -o ./mt1.json -f ./mtip.txt
            enhanced_args.append("-n mongodb")
            ...
            # outfi = os.path.join(self._tmpdir, "{}_{}.mongodb".format(task.taskid, port))
            with self._outfile_locker:
                outfi = os.path.join(
                    self._tmpdir, "{}_{}.mongodb".format(str(uuid.uuid1()), port)
                )
            ...
            try:
                ...
                if exitcode != 0:
                    raise Exception(f"Scan Mongodb error: {stdout}\n{stderr}")
                self._logger.info(
                    f"Scan Mongodb exitcode={str(exitcode)}\ntaskid:{task.taskid}\nbatchid:{task.batchid}\nport:{port}"
                )
            finally:
