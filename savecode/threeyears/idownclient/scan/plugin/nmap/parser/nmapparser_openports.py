...
# -*- coding:utf-8 -*-
from idownclient.clientdatafeedback.scoutdatafeedback import portinfo
...
class NmapParserOpenPorts:
    ...
    @classmethod
    def parse_open_ports(cls, task: IscanTask, level: int, nmap_result_fi: str) -> iter:
        """yield Port"""
        try:
    ...
    @classmethod
    def _get_ports(cls, xhost: etree._Element, task: IscanTask, level: int) -> iter:
        ...
        for xp in xports:
            try:
                ...
                cls._logger.debug("Get ports warn: {}".format(traceback.format_exc()))
    ...
    def _get_hostnames(cls, xhost: etree._Element) -> list:
        ...
        try:
            ...
            cls._logger.debug("Get hostnames error:{}".format(traceback.format_exc()))
        return res

    @classmethod
    def _get_os(cls, xhost: etree._Element):
    ...
    @classmethod
    def parse_udp_10001(cls, task: IscanTask, level: int, nmap_result_fi: str):
        """yield Port"""
        try:
            doc: etree._ElementTree = etree.parse(nmap_result_fi)
            infos: list = doc.xpath("//host")
            for info in infos:
                if info is None:
                    continue
    ...
    @classmethod
    def _get_udp_10001_portinfo(
        cls, info: etree._Element, task: IscanTask, level: int
    ) -> iter:
        try:
            # 1: hostname
            ...
            cls._logger.error(
                "Parse udp 10001 result error: {}".format(traceback.format_exc())
            )

    @classmethod
    def _get_udp_10001_hostname(cls, info: etree._Element):
        hostname = info.xpath('ports/port/script/elem[@key="hostname"]')[0].text
        return hostname

    @classmethod
    def _get_udp_10001_product(cls, info: etree._Element):
        product = info.xpath('ports/port/script/elem[@key="product"]')[0].text
        return product

    @classmethod
    def _get_udp_10001_version(cls, info: etree._Element):
        version = info.xpath('ports/port/script/elem[@key="firmware"]')[0].text
        return version
    ...
    def _get_udp_10001_ip_mac(cls, info: etree._Element):
        ...
        ip = info.xpath("address")[0].get("addr")

        addresses = info.xpath("ports/port/script/table")[0]
