...
新增直接使用GeoIPLoc by judy 2020/04/23
"""
import traceback
...
class DbipMmdb(object):
    ...
    def format_string(self, instr):
        ...
        try:
            ...
            self._logger.error(
                f"Format dbipinfo {instr} error, err:{traceback.format_exc()}"
            )
        return res
    ...
    def get_ip_mmdbinfo(self, level: int, ip: str):
        ...
        try:
            ...
            if len(subdivisions) > 0:
                ...
                data_state_cn = self.format_string(
                    data_state_info.get("names", {}).get("zh-CN", None)
                )
            # countryinfo
            ...
            data_country_is_in_eu_union = data_country_info.get(
                "is_in_european_union", False
            )
            data_country_en = data_country_info.get("names", {}).get("en")
            data_country_cn = self.format_string(
                data_country_info.get("names", {}).get("zh-CN")
            )
            # location info
            ...
            if (
                not ALL_COUNTRIES.__contains__(data_country_code)
                or lat is None
                or lng is None
            ):
                return res, org, isp
            ...
            geoinfo.set_country(
                data_country_code,
                data_country_geoid,
                data_country_is_in_eu_union,
                {"zh-CN": data_country_cn, "en": data_country_en},
            )
            geoinfo.set_province(
                {"zh-CN": data_state_cn, "en": data_state_en},
                code=data_state_isocode,
                geoid=data_state_geoid,
            )
            geoinfo.set_city(
                {"zh-CN": data_city_cn, "en": data_city_en}, geoid=data_city_geoid
            )
            # 大洲
            ...
            geoinfo.set_continent(
                ctcode, ctgeoid, {"zh-CN": ctname_cn, "en": ctname_en}
            )
            ...
            if res_dict is not None:
                self._logger.error(
                    "Get geoinfo error: ip={}, error: {}".format(
