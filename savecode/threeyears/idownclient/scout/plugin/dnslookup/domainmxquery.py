...
class DomainMXQuery(ScoutPlugBase):
    ...
    def get_mail_server(self, level, domain, reason=None):
        """
        外部接口，根据domain去获取mx记录
        和邮箱的差不多，但是为了以防万一，多写一份代码来修改
        :param level:
        :param domain:
        :param reason:
        :return:
        ...
        proc = Popen(f'nslookup -q=mx {domain}', stdout=PIPE, shell=True)
        outs, errs = proc.communicate(timeout=15)
        # 根据不同的操作系统用不同的编码解析
        res = outs
        if self.pf == 'linux':
        ...
        for md in ms_list:
            ...
            ip = self.__ping_domain(ms)
            if ip is not None:
                ms.set_ip(ip)
            self._logger.debug(f"Get mail server successed")
            yield ms