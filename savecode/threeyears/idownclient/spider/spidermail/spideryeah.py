...
class SpiderYeah(SpiderMailBase):

    def __init__(self, task, appcfg, clientid):
        super(SpiderYeah, self).__init__(task, appcfg, clientid)
        self._html = None
        self.sid = None
    ...
    def _pwd_login(self) -> bool:
        ...
        try:
            # 获取pkid
            url = 'https://mimg.127.net/p/freemail/index/unified/static/2020/js/yeah.be4b6fb4f5077778b166.js'
            headers = '''
            Referer: https://mail.yeah.net/
            ...
            ini_url = 'https://passport-v6.yeah.net/dl/ini?pd=mailyeah&pkid={}&pkht=mail.yeah.net'.format(pkid)
            self._ha.getstring(ini_url)

            # 获取tk参数
            un = urllib.parse.quote_plus(self.task.account)
            gt_url = 'https://passport-v6.yeah.net/dl/gt?un={}&pkid={}&pd=mailyeah'.format(un, pkid)
            headers = '''
            Accept: */*
            Accept-Encoding: gzip, deflate, br
            Accept-Language: zh-CN,zh;q=0.9
            Connection: keep-alive
            Host: passport-v6.yeah.net
            ...
            # 提交账号密码
            l_url = 'https://passport-v6.yeah.net/dl/l'
            payload = {
                'channel': '0',
                'd': '10',
                'domains': '163.com',
            ...
            Connection: keep-alive
            Host: passport-v6.yeah.net
            Origin: https://passport-v6.yeah.net
            User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.121 Safari/537.36
            '''
            ...
            # 登陆主页
            ntesdoor_url = 'https://mail.yeah.net/entry/cgi/ntesdoor?'
            data = '''
            style: -1
            df: mail163_letter
            allssl: true
            ...
            race: 
            iframe: 1
            url2: https://mail.yeah.net/errorpage/err_yeah.htm
            product: mailyeah'''
            headers = '''
    ...
    def _cookie_login(self) -> bool:
        res = False
        try:
            self._ha._managedCookie.add_cookies('.yeah.net', self.task.cookie)
            self.sid = self._get_sid()
            if self.sid is None:
                self._logger.error("Coremail.Cookie not found")
                return res

            url = f'http://mail.yeah.net/js6/main.jsp?sid={self.sid}'
    ...
    def _get_profile(self) -> iter:
        ...
        if self._html is None:
            ...
            url = "https://mail.yeah.net/js6/main.jsp"
    ...
    def __download_eml(self, mail_id):
        url = "https://mail.yeah.net/js6/read/readdata.jsp"
        # sid = self._get_sid()
        if self.sid is None:
            self._logger.error("Invalid cookie")
